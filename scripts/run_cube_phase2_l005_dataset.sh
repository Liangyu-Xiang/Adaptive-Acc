...
if [[ $# -ne 2 ]]; then
    echo "usage: $0 {tum|7scenes} PHYSICAL_GPU" >&2
    exit 2
fi
...
run_root=${repo_root}/outputs/cube_phase2_l005_300
...
echo "[$(date -Is)] dataset=${dataset} lambda=0.05 physical_gpu=${physical_gpu}" > "${log_file}"
if [[ ${dataset} == tum ]]; then
    exec env \
        VGGT_UM_TRITON=1 \
        PYTHONUNBUFFERED=1 \
