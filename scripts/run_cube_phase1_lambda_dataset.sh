...
if [[ $# -ne 3 ]]; then
    echo "usage: $0 {tum|7scenes} LAMBDA PHYSICAL_GPU" >&2
    exit 2
fi
...
lambda_tag=${lambda/./p}
run_root=${repo_root}/outputs/cube_phase1_lambda_300
output_dir=${run_root}/${dataset}_lambda_${lambda_tag}
log_dir=${run_root}/logs
log_file=${log_dir}/${dataset}_lambda_${lambda_tag}_gpu${physical_gpu}.log
...
echo "[$(date -Is)] dataset=${dataset} lambda=${lambda} physical_gpu=${physical_gpu}" > "${log_file}"
if [[ ${dataset} == tum ]]; then
    exec env \
        PYTHONUNBUFFERED=1 \
