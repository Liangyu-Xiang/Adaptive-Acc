#!/usr/bin/env bash
# One formal task: >=1000-frame sequences use uniform-1000 then its first 300;
# shorter sequences use their first 300 source frames.
set -euo pipefail
gpu=${1:?gpu}; model=${2:?omega|pi3|vggt}; method=${3:?method}; dataset=${4:?tum|7scenes}
root=/data/mmc_syang
out=${root}/VGGT-omega/outputs/compact1000_first300_formal/${model}/${method}/${dataset}
mkdir -p "${out}"
...
if [[ ${model} == omega ]]; then
  ...
  cd ${root}/VGGT-omega
  args=(--data-root "${data_root}" --checkpoint ${root}/vggt-omega/checkpoints/vggt_omega_1b_512.pt --output-dir "${out}" --device cuda:0 --num-frames 300 --sampling-pool-frames 1000 --image-resolution 512 --resize-mode max_size --sampling-strategy uniform --timing-repeats 3)
  ...
  [[ ${dataset} == tum ]] && data_root=${root}/dataset/TUM-Dynamics || data_root=${root}/dataset/7scenes
  cd ${root}/Pi3
  CUDA_VISIBLE_DEVICES=${gpu} VGGT_UM_TRITON=1 PYTHONPATH=${root}/Pi3 ${root}/miniconda3/envs/flow3r/bin/python scripts/run_pi3_vggt_omega_eval.py --dataset "${data}" --dataset-root "${data_root}" --pretrained checkpoints/Pi3 --output-dir "${out}" --device cuda:0 --max-frames-per-seq 300 --frame-sample-mode uniform --sampling-pool-frames 1000 --load-img-size 512 --timing-repeats 3 --acceleration-method "${method}" --token-merging-ratio 0.9 --sparse-vggt-sparse-ratio 0.5 --da-chunk-size 50 --overwrite
else
  [[ ${dataset} == tum ]] && data=tum_dynamic || data=7scenes
  [[ ${dataset} == tum ]] && data_root=${root}/dataset/TUM-Dynamics || data_root=${root}/dataset/7scenes
  cd ${root}/vggt
  CUDA_VISIBLE_DEVICES=${gpu} VGGT_UM_TRITON=1 PYTHONPATH=${root}/vggt ${root}/miniconda3/envs/fastvggt/bin/python scripts/eval_standard_tum_7scenes.py --dataset "${data}" --dataset-root "${data_root}" --checkpoint ckpts/model.pt --output-dir "${out}" --device cuda:0 --num-frames 300 --sampling-pool-frames 1000 --image-resolution 518 --timing-repeats 3 --acceleration-method "${method}" --merge-ratio 0.9 --sparse-vggt-sparse-ratio 0.5 --da-chunk-size 50 --overwrite
fi
